...
def clean_dataset(input_path, output_path):
    ...
    """
    logging.info(f"Starting dataset cleaning: {input_path}")
    logging.info("Strategy: Delete only rows missing customer_id, fill all other fields")
    ...
    logging.info("Trimmed column names")
    
    for col in df.select_dtypes(include="object"):
        df[col] = df[col].astype(str).str.strip()
    logging.info("Trimmed whitespace from object columns")
    ...
    if deleted > 0:
        logging.warning(f"Deleted {deleted} rows with missing customer_id")
    
    logging.info(f"After deleting rows with missing customer_id: {len(df)}/{initial_count} rows remain")
    ...
    deleted = before - len(df)
    if deleted > 0:
        logging.warning(f"Deleted {deleted} rows with invalid customer_id")
    
    logging.info("Converted customer_id and income to numeric")
    ...
    if first_name_missing > 0:
        logging.info(f"Filled {first_name_missing} missing first_name with 'Unknown'")
    ...
    if last_name_missing > 0:
        logging.info(f"Filled {last_name_missing} missing last_name with 'Unknown'")
    ...
    if email_missing > 0:
        logging.info(f"Filled {email_missing} missing email with 'noemail@placeholder.com'")
    ...
    logging.info("Normalized first_name and last_name to title case")
    ...
    if phone_missing > 0:
        logging.info(f"Filled {phone_missing} missing phone numbers with placeholder '000-000-0000'")
    ...
    if dob_missing > 0:
        logging.info(f"Filled {dob_missing} missing date_of_birth with placeholder '1900-01-01'")
    
    created_missing = df["created_date"].isna().sum()
    df["created_date"] = df["created_date"].fillna(pd.Timestamp.now())
    if created_missing > 0:
        logging.info(f"Filled {created_missing} missing created_date with current date")
    ...
    if address_missing > 0:
        logging.info(f"Filled {address_missing} missing addresses with placeholder")
    
    # FILL: Replace missing income with 0.0
    income_missing = df["income"].isna().sum()
    df["income"] = df["income"].fillna(0.0)
    if income_missing > 0:
        logging.info(f"Filled {income_missing} missing income values with 0.0")
    ...
    if status_missing > 0:
        logging.info(f"Filled {status_missing} missing account_status with 'inactive'")
    ...
    for index, row in df.iterrows():
        try:
            ...
            logging.warning(f"Row {index} validation failed after cleaning: {e}")
            flagged_rows.append({
                "row_index": index,
            ...
            logging.warning(f"Row {index} type conversion failed: {e}")
            flagged_rows.append({
                "row_index": index,
                "customer_id": row.get("customer_id"),
                "error": str(e)
    ...
    logging.info(f"  Deleted (missing customer_id): {deleted_count}")
    logging.info(f"  Cleaned successfully: {cleaned_count}")
    logging.info(f"  Failed validation: {flagged_count}")
    ...
    logging.info("Reset index for cleaned data")
    ...
    try:
        ...
        logging.info(f"Saved {cleaned_count} cleaned rows to {output_path}")
    except Exception as e:
